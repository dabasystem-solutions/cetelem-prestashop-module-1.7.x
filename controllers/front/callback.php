...
class CetelemCallbackModuleFrontController extends ModuleFrontController
{
    ...
    private const LOG_FILE_PATH = _PS_MODULE_DIR_ . 'cetelem/registro.log';
    ...
    private function isTransactionDuplicate($idTransaccion)
    {
        $query = new DbQuery();
        $query->select('COUNT(*)');
    ...
    private function processTransaction()
    {
        ...
        $id_cart = (int)Tools::substr($idTransaccion, 4);
        ...
        if ($this->isTransactionDuplicate($idTransaccion)) {
            $this->writeToLog("Duplicate transaction detected: $idTransaccion");
            $this->sendStatus(1, $existing_order);
            return;
        }

        $this->saveTransactionToDatabase($idTransaccion, $codResultado, $id_cart);
